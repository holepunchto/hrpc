module.exports = function generateCode (hrpc, { esm = false }) {
  ...
  if (esm) {
    str += 'import { c, RPC, RPCStream, RPCRequestStream } from \'hrpc/runtime\'\n'
    str += 'import { getEncoding } from \'./messages.js\'\n'
  } else {
    str += 'const { c, RPC, RPCStream, RPCRequestStream } = require(\'hrpc/runtime\')\n'
  ...
  hrpc.handlers.filter(e => !!e.response).forEach((e, i, arr) => {
    if (i < arr.length - 1) {
  ...
  str += '        const request = req.data ? c.decode(requestEncoding, req.data) : null\n'
  ...
  hrpc.handlers.forEach((e) => {
    const returnsStream = e.response?.stream || e.request.stream || e.request.send
  ...
  str += '    return [\n'
  hrpc.handlers.filter(e => e.request.stream === true).forEach((e, i, arr) => {
    if (i < arr.length - 1) {
      str += `      '${e.name}',\n`
    } else {
      str += `      '${e.name}'\n`
  ...
  str += '    return [\n'
  hrpc.handlers.filter(e => e.response?.stream === true).forEach((e, i, arr) => {
    if (i < arr.length - 1) {
      str += `      '${e.name}',\n`
    } else {
      str += `      '${e.name}'\n`
  ...
  str += '    return [\n'
  hrpc.handlers.filter(e => e.request.send === true).forEach((e, i, arr) => {
    if (i < arr.length - 1) {
      str += `      '${e.name}',\n`
    } else {
      str += `      '${e.name}'\n`
