module.exports = function generateCode (hyperInterface, { esm = false }) {
  ...
  if (esm) {
    str += 'import { c, RPC, RPCStream, RPCRequestStream } from \'hyperinterface/runtime\'\n'
    str += 'import { getEncoding } from \'./messages.js\'\n'
  } else {
    str += 'const { c, RPC, RPCStream, RPCRequestStream } = require(\'hyperinterface/runtime\')\n'
  ...
  hyperInterface.handlers.filter(e => !!e.response).forEach((e, i) => {
    if (i < hyperInterface.handlers.length - 1) {
  ...
  str += '    return [\n'
  hyperInterface.handlers.filter(e => e.request.send === true).forEach((e, i) => {
    if (i < hyperInterface.handlers.length - 1) {
      str += `      '${e.name}',\n`
    } else {
      str += `      '${e.name}'\n`
