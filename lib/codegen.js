...
module.exports = function generateCode(hrpc, { esm = false, flat = false }) {
  let str = ''
  ...
  str += '    })\n'
  if (!flat) {
    const groups = groupByNamespace(hrpc.handlers)
    const namespaces = [...groups.keys()].filter((k) => k !== null)
    if (namespaces.length > 0) {
      str += '    const self = this\n'
      for (const ns of namespaces) {
        const handlers = groups.get(ns)
        const nsName = convertToCamelCase(ns)
        str += `    this.${nsName} = {\n`
        handlers.forEach((e) => {
          const methodName = convertToCamelCase(e.name)
          const returnsStream =
            e.response?.stream || e.request.stream || e.request.send
          str += `      ${returnsStream ? '' : 'async '}${methodName}(args) { return self.${returnsStream ? '_callSync' : '_call'}('${e.name}', args) },\n`
        })
        handlers.forEach((e) => {
          const onMethodName = convertToCamelCase(e.name, { prefix: 'on' })
          str += `      ${onMethodName}(responseFn) { self._handlers['${e.name}'] = responseFn },\n`
        })
        str += '    }\n'
      }
    }
  ...
  str += '  }\n'
  str += '\n'
  const classMethodHandlers = flat
    ? hrpc.handlers
    : hrpc.handlers.filter((e) => !e.name.startsWith('@'))
  classMethodHandlers.forEach((e) => {
    const returnsStream =
      e.response?.stream || e.request.stream || e.request.send
...
function parseNamespace(fqn) {
  if (fqn.startsWith('@')) {
    const slashIndex = fqn.indexOf('/')
    if (slashIndex !== -1) {
      return {
        namespace: fqn.slice(1, slashIndex),
        method: fqn.slice(slashIndex + 1)
      }
    }
  }
  return { namespace: null, method: fqn }
}

function groupByNamespace(handlers) {
  const groups = new Map()
  for (const handler of handlers) {
    const { namespace } = parseNamespace(handler.name)
    if (!groups.has(namespace)) groups.set(namespace, [])
    groups.get(namespace).push(handler)
  }
  return groups
}
