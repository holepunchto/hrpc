module.exports = function generateCode (hrpc, { esm = false }) {
  ...
  hrpc.handlers.filter(e => !!e.response).forEach((e, i) => {
    if (i < hrpc.handlers.length - 1) {
  ...
  str += '    return [\n'
  hrpc.handlers.filter(e => e.request.stream === true).forEach((e, i) => {
    if (i < hrpc.handlers.length - 1) {
      str += `      '${e.name}',\n`
    } else {
      str += `      '${e.name}'\n`
  ...
  str += '    return [\n'
  hrpc.handlers.filter(e => e.response?.stream === true).forEach((e, i) => {
    if (i < hrpc.handlers.length - 1) {
      str += `      '${e.name}',\n`
    } else {
      str += `      '${e.name}'\n`
  ...
  str += '    return [\n'
  hrpc.handlers.filter(e => e.request.send === true).forEach((e, i) => {
    if (i < hrpc.handlers.length - 1) {
      str += `      '${e.name}',\n`
    } else {
      str += `      '${e.name}'\n`
