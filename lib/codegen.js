module.exports = function generateCode (hyperInterface, { esm = false }) {
  ...
  if (esm) {
    str += 'import { c, RPC } from \'hyperinterface/runtime\'\n'
    str += 'import { getEncoding } from \'./messages.js\'\n'
  } else {
    str += 'const { c, RPC } = require(\'hyperinterface/runtime\')\n'
  ...
  hyperInterface.handlers.forEach((e) => {
    str += `  async ${convertToCamelCase(e.name, { prefix: 'on', cInterfacetalizeFirst: true })} (responseFn) {\n`
...
function convertToCamelCase (input, opts = { prefix: '', cInterfacetalizeFirst: false }) {
  if (input.startsWith('@')) {
    input = input.slice(1)
  }
  ...
  const camelCased = parts.map((part, index) => {
    if (index === 0 && !opts.cInterfacetalizeFirst) return part
